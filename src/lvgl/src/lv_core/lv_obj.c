...
lv_font_t *LV_FONT_SIZE_16 = NULL;
lv_font_t *LV_FONT_SIZE_20 = NULL;
lv_font_t *LV_FONT_SIZE_22 = NULL;
...
lv_font_t *LV_FONT_SIZE_60 = NULL;
lv_font_t *LV_FONT_SIZE_100 = NULL;
lv_font_t *LV_FONT_SIZE_120 = NULL;
lv_font_t *LV_FONT_SIZE_160 = NULL;
...
const unsigned char *get_rom_bin_base(void)
{
    ...
    if (rom_bin_base == NULL)
    {
        ...
        {
            sprintf(file_path, "%s", ROM_BIN_PATH);
        }
        else
        {
            sprintf(file_path, "%s", ROM_BIN_TEMP_PATH);
        }

        int ret = stat(file_path, &statbuf); // 调用stat函数
        if (ret != 0)
        {
            LV_LOG_ERROR("get %s fail,ROM_BIN_PATH:%s\n", file_path, ROM_BIN_PATH);
            return NULL;
        }
...
void lv_init(void)
{
    ...
    lv_ft_font_init(&LV_FONT_SIZE_16, 16);
    lv_ft_font_init(&LV_FONT_SIZE_20, 20);
    lv_ft_font_init(&LV_FONT_SIZE_22, 22);
    ...
    lv_ft_font_init(&LV_FONT_SIZE_60, 60);
    lv_ft_font_init(&LV_FONT_SIZE_100, 100);
    lv_ft_font_init(&LV_FONT_SIZE_120, 120);
    lv_ft_font_init(&LV_FONT_SIZE_160, 160);
...
#if LV_USE_BIDI
    const lv_obj_t *parent = obj;
    // printf("lv_obj_get_base_dir parent :%p    parent->base_dir:%d\n",parent,parent->base_dir);
    while (parent)
    {
...
void lv_obj_init_draw_rect_dsc(lv_obj_t *obj, uint8_t part, lv_draw_rect_dsc_t *draw_dsc)
{
    ...
    draw_dsc->base_dir = obj->base_dir;
#if LV_USE_OPA_SCALE
...
static lv_design_res_t lv_obj_design(lv_obj_t *obj, const lv_area_t *clip_area, lv_design_mode_t mode)
{
    ...
    else if (mode == LV_DESIGN_DRAW_MAIN)
    {
        ...
        // printf("%s obj :%p     base_dir:%d\n",__func__,obj,obj->base_dir);
