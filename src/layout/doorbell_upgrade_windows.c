 ...
 * @LastEditTime: 2024-07-26 13:48:55
...
#define IPADDR_BASE "192.168.37."
#define UPGRADE_MAX 25
#define UPGRADE_DEVICE_NUM 2

static void ListFlush(lv_obj_t *obj, int list_type, int dev_type);
#define IMG_DEFINE(IMG, DEV_TYPE, IMG_TYPE)                     \
    {                                                           \
        .name = IMG, .dev_type = DEV_TYPE, .img_type = IMG_TYPE \
    }
...
typedef struct
{
    char name[32];
    char path[128];
    upgrade_img_type img_type;
    upgrade_dev_type dev_type;
} upgrade_file_info;
...
typedef struct
{
    ...
    int id;
    char path[128];
    upgrade_dev_type type;
} upgrade_dev_info;

static char *path_group[TOTAL_PATH] = {
    "/app/app/", "/mnt/tf/"};

static upgrade_file_info file_info[TOTAL_PATH][6] = {
    {
        IMG_DEFINE("330L-82227-EPC", DOORBELL_TYPE, KERNEL_IMG),
        IMG_DEFINE("330L-2CD-ME2NP", DOORBELL_TYPE, KERNEL_IMG),
        IMG_DEFINE("SAT_ANYKA_AV100.IMG", DOORBELL_TYPE, UBOOT_IMG),
        IMG_DEFINE("SAT_ANYKA_330L.IMG", DOORBELL_TYPE, UBOOT_IMG),
    },

    {
        ...
        IMG_DEFINE("SAT_ANYKA_AV100.IMG", DOORBELL_TYPE, UBOOT_IMG),
        IMG_DEFINE("SAT_ANYKA_330L.IMG", DOORBELL_TYPE, UBOOT_IMG),
        IMG_DEFINE("SAT_ANYKA_3761E.IMG", INDOOR_TYPE, UBOOT_IMG),
        IMG_DEFINE("SAT_ANYKA_3760E.IMG", INDOOR_TYPE, UBOOT_IMG),
    },
};
...
struct upgrade_file_list
{
    list_base base;
    upgrade_dev_type support_dev;
    upgrade_file_info info[UPGRADE_MAX];
} file_list;

struct dev_type_list
{
    list_base base;
    upgrade_dev_info info[8];
} dev_list;

static int list_type = 0;

static int SearchUpgradeFile(char *path, upgrade_file_info file_info)
{
    DIR *dir;
    struct dirent *entry;
    ...
    {
        // Check if entry is a regular file and starts with file_info.img
        if (entry->d_type == DT_REG && strncmp(entry->d_name, file_info.name, strlen(file_info.name)) == 0)
        {
            // Debug("file_info.name:%s,dev_type:%d\n", file_info.name, file_info.dev_type);
            // Allocate memory for file name and copy it
            sprintf(file_list.info[file_list.base.num].path, "%s", path);
            sprintf(file_list.info[file_list.base.num].name, "%s", entry->d_name);
            file_list.info[file_list.base.num].dev_type = file_info.dev_type;
            file_list.info[file_list.base.num].img_type = file_info.img_type;
            file_list.base.num++;
            file_list.support_dev |= (1 << file_info.dev_type);
            // Break loop if maximum number of files reached
            if (file_list.base.num >= UPGRADE_MAX)
            {
                break;
            }
        }
...
static int RetrieveAllUpgradePath(void)
{
    ...
    int model_num = sizeof(file_info[0]) / sizeof(file_info[0][0]);
    ...
    for (int i = 0; i < path_num; i++)
    {
        ...
        {
            SearchUpgradeFile(path_group[i], file_info[i][j]);
        }
    }
    Debug("%d upgrade files were retrieved\n", file_list.base.num);
    for (int x = 0; x < file_list.base.num; x++)
    {
        Debug("%d. path:%s%s\n", x, file_list.info[x].path, file_list.info[x].name);
    }
    return file_list.base.num;
}
...
static int SearchUpgradeDevice(void)
{
    ...
    char *door[] = {"Door", "门口机", "Türstation ", "דלת  ", "Drzwi ", "Porta", "Puerta", "Platine ", "玄関ドアフォン", "Porta"};
    char *indoor[] = {"Indoor", "室内机", "Innenstation ", "יחידה פנימית", "Jednostka wewnętrzna ", "Unidade interior", "Máquina interior", "Machine intérieure ", "室内機", "Unità interna"};
    for (int i = DEVICE_INDOOR_ID1; i < DEVICE_CCTV_1; i++)
    {
        if (i < DEVICE_OUTDOOR_1 && (file_list.support_dev & (1 << INDOOR_TYPE)))
        {
            sprintf(dev_list.info[dev_list.base.num].name, "%s%d", indoor[user_data_get()->other.language], i);
            sprintf(dev_list.info[dev_list.base.num].path, "%s%d", IPADDR_BASE, user_data_get()->other.family_id * 10 + i);
            dev_list.info[dev_list.base.num].type = INDOOR_TYPE;
            // Debug("%d.%s %s %d\n", dev_list.base.num, dev_list.info[dev_list.base.num].name, dev_list.info[dev_list.base.num].path, file_list.support_dev);
            dev_list.base.num++;
        }
        else if (i >= DEVICE_OUTDOOR_1 && file_list.support_dev & (1 << DOORBELL_TYPE))
        {
            sprintf(dev_list.info[dev_list.base.num].name, "%s%d", door[user_data_get()->other.language], i - DEVICE_OUTDOOR_1 + 1);
            sprintf(dev_list.info[dev_list.base.num].path, "%s%d", IPADDR_BASE, i);
            dev_list.info[dev_list.base.num].type = DOORBELL_TYPE;
            // Debug("%d.%s %s %d\n", dev_list.base.num, dev_list.info[dev_list.base.num].name, dev_list.info[dev_list.base.num].path, file_list.support_dev);
            dev_list.base.num++;
        }
    }

    Debug("dev_list.base.num:%d\n", dev_list.base.num);
    return dev_list.base.num;
}
...
static void ListBtnUp(lv_obj_t *obj)
{
    ...
    if (*list_slide == false)
    {
        ...
        case 0:
            ...
            Debug("%s type:%d\n", dev_list.info[dev_id].name, dev_list.info[dev_id].type);
            ListFlush(list, ++list_type, dev_list.info[dev_id].type);
            break;

        case 1:
        {
            char path[128] = {0};
            sprintf(path, "%s%s", file_list.info[obj->obj_id].path, file_list.info[obj->obj_id].name);
            Debug("%d.%s %s %s %d\n", obj->obj_id, dev_list.info[dev_id].name, dev_list.info[dev_id].path, path, file_list.info[obj->obj_id].img_type);
            lv_obj_del(list->parent);
            // lv_obj_del(list->parent->parent);
            UpgradeWindowCreate(list->parent->parent, "Upgradeing ....", true);
            CreateUpgradeClientTask(dev_list.info[dev_id].path, file_list.info[obj->obj_id].img_type ? file_list.info[obj->obj_id].name : path, file_list.info[obj->obj_id].img_type);
        }
        break;
        default:
            break;
...
static void ListFlush(lv_obj_t *obj, int list_type, int dev_type)
{
    ...
    int i = 0;
    int j = list_type ? file_list.base.num : dev_list.base.num;
    char *name = NULL;
    for (; i < UPGRADE_MAX; i++, j--)
    {
        if (j == 0)
        {
            break;
        ...
        name = list_type ? file_list.info[i].name : dev_list.info[i].name;
        if (list_type)
        {
            // Debug("dev_type:%d,name :%s dev_type:%d\n", dev_type, file_list.info[i].name, file_list.info[i].dev_type);
            if (dev_type == INDOOR_TYPE && (file_list.info[i].dev_type != (INDOOR_TYPE)))
            {
                continue;
            } /*  */
            else if (dev_type == DOORBELL_TYPE && (file_list.info[i].dev_type != (DOORBELL_TYPE)))
            {
                continue;
            }
        }
        if (ListCreateFunc(obj, (i)*LIST_BTN_HEIGHT, LIST_BTN_HEIGHT, i, name) == NULL)
        {
            i--;
        }
    }
    ...
    {
        ListCreateFunc(obj, (i)*LIST_BTN_HEIGHT, LIST_BTN_HEIGHT * (LIST_ONCE_DISPLAY_MAX - i), -1, name);
        ListCreateFunc(obj, (i)*LIST_BTN_HEIGHT, 10, -1, name);
    }
}
