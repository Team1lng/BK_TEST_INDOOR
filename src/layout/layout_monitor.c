...
static bool monitor_background_ui_exit = false;
...
{
	monitor_background_ui_exit = true;
}

static void monitor_tuya_background_session_close(void)
{
	Debug("[TUYA_AUDIO_TRACE] tuya background close: channel=%d enter_way=%d clients=%d tuya_talk=%d local_talk=%d\n",
		  monitor_channel_get(), monitor_enter_way_get(), tuya_client_num_get(), is_tuya_talking, is_talking);
	Debug("[TUYA_AUDIO_TRACE] tuya background close -> audio_talk_close(true)\n");
	audio_talk_close(true);
	if (is_tuya_talking || is_talking)
	{
		Debug("[TUYA_AUDIO_TRACE] tuya background close -> send_monitor_talk_cmd(false)\n");
...
static void monitor_timer_task(struct _lv_task_t *task_t)
{
	...
	{
		int talk_vol = monitor_channel_get() == MON_CH_DOOR_1 ? user_data_get()->door1.talk_volume * 5 + 46 : user_data_get()->door2.talk_volume * 5 + 46;
	...
	if ((sec = !sec) && monitor_enter_way_get() != MONITOR_ENTER_TUYA)
	{
		...
		{
			Debug("[TUYA_AUDIO_TRACE] monitor timeout -> standby: channel=%d enter_way=%d clients=%d tuya_talk=%d local_talk=%d\n",
				  monitor_channel_get(), monitor_enter_way_get(), tuya_client_num_get(), is_tuya_talking, is_talking);
...
static void monitor_indoor_cmd_func(unsigned long arg1, unsigned long arg2)
{
	...
	{
		if (tuya_session_should_ignore_indoor_busy(monitor_enter_way_get() == MONITOR_ENTER_TUYA))
		{
			break;
		}
...
void tuya_switch_camera(char channel)
{
	...
	if (ch != channel)
	{
		...
		if (channel < MON_CH_CCTV_1)
		{
			...
			network_device devcie = channel + DEVICE_INDOOR_ID6;
			audio_talk_ctrl ctrl = {{devcie}, (OPERATION_OPTION(AUDIO_SEND_EN) | OPERATION_OPTION(AUDIO_RECEIVE_EN)), AI_AO_C, true, false, channel == MON_CH_DOOR_1 ? user_data_get()->door1.talk_volume * 5 + 46 : user_data_get()->door2.talk_volume * 5 + 46};
	...
	tuya_channel_valid_report(); // tuya_switch_channel_upload_results(channel);
}
...
void tuya_event_inside_proc(unsigned long arg1, unsigned long arg2)
{
	...
	{
		if (current_layout_get() != &layout_standby && current_layout_get() != &layout_monitor && monitor_enter_way_get() != MONITOR_ENTER_TUYA)
		{
			goto_layout(pLAYOUT(standby));
		}
	}
...
void monitor_status_callback(unsigned long arg1, unsigned long arg2)
{
	if (monitor_channel_get() == MON_CH_DOOR_1 || monitor_channel_get() == MON_CH_DOOR_2)
	{
		network_device dev = monitor_channel_get() == MON_CH_DOOR_1 ? DEVICE_OUTDOOR_1 : DEVICE_OUTDOOR_2;
		if (!device_online_state_get(dev))
		{
...
static void monitor_enter_parameter_init(void)
{
	...
	dev_info_status_event_register(monitor_status_callback);
...
void monitor_setting_window_create(void)
{
...
static void monitor_tuya_busy_msgbox_create(void)
{
	lv_obj_t *msg = msgbox_animat_create(text_str(STR_PHONE_MONITORING), 1000);
	lv_obj_set_pos(msg, 350, 187);
	lv_obj_set_size(msg, 324, 226);
}
...
{
	// extern int tuya_ipc_get_client_online_num();
	send_monitor_hang_cmd();
...
static void device_monitor_busy_func(unsigned long arg1, unsigned long arg2)
{
	...
	if (current_layout_get() != &layout_standby && monitor_enter_way_get() != MONITOR_ENTER_TUYA && !is_talking)
	{
		goto_layout(pLAYOUT(standby));
	}
}
...
static void LAYOUT_QUIT_FUNC(monitor)
{
	...
	bool keep_tuya_video = tuya_session_keep_shared_video(tuya_client_active);
	bool keep_tuya_audio = tuya_session_keep_shared_media(
		tuya_client_active,
		monitor_enter_way_get() == MONITOR_ENTER_TUYA);

	Debug("[TUYA_AUDIO_TRACE] monitor layout quit: background_exit=%d channel=%d enter_way=%d clients=%d tuya_talk=%d local_talk=%d\n",
		  monitor_background_ui_exit, monitor_channel_get(), monitor_enter_way_get(), tuya_client_num_get(), is_tuya_talking, is_talking);
	if (monitor_background_ui_exit)
	{
		monitor_background_ui_exit = false;

		if (monitor_time_ptask != NULL)
		{
			lv_task_del(monitor_time_ptask);
			monitor_time_ptask = NULL;
		}
		if (monitor_timer_ptask != NULL)
		{
			lv_task_del(monitor_timer_ptask);
	...
	if (keep_tuya_video)
	{
		Debug("[TUYA_AUDIO_TRACE] monitor layout quit: preserve tuya video path only\n");
		// 室内机退出监控页时，保留门口机接收通道供手机涂鸦继续取流。
		fb_video_mode_enable(false);
		video_decode_close();
	}
	else
	...
	if (keep_tuya_audio)
	{
		Debug("[TUYA_AUDIO_TRACE] monitor layout quit: preserve tuya audio, skip audio close and hang command\n");
	}
	else
	{
		Debug("[TUYA_AUDIO_TRACE] monitor layout quit -> audio_talk_close(true): clients=%d tuya_talk=%d local_talk=%d\n",
			  tuya_client_num_get(), is_tuya_talking, is_talking);
		audio_talk_close(true); // 音频关闭
		if (is_tuya_talking || is_talking)
		{
			Debug("[TUYA_AUDIO_TRACE] monitor layout quit -> send_monitor_talk_cmd(false)\n");
			send_monitor_talk_cmd(false);
		}
		is_tuya_talking = false;
	}
	...
	if (timeval.sec != sec)
	{
		...
		if (monitor_ring_timeout_val == 0)
		{
			...
			if (monitor_channel_get() < MON_CH_CCTV_1)
			{
				...
				{
					// Debug("monitor_timeout_val:%d  curr_monitor_ch.message_time:%d\n", monitor_timeout_val, curr_monitor_ch.message_time);
					{
						int free_space = sd_free_space_insufficient();
						if (free_space > 200)
						{
...
static void mechanical_key_inside_callback(unsigned long arg1, unsigned long arg2)
{
	...
	audio_talk_close(false);
	{
		audio_talk_ctrl ctrl = {{ch+DEVICE_INDOOR_ID6},(OPERATION_OPTION(AUDIO_SEND_EN)|OPERATION_OPTION(AUDIO_RECEIVE_EN)),
													AI_AO_C,true,false,ch == MON_CH_DOOR_1 ?  user_data_get()->door1.talk_volume*5 + 46:user_data_get()->door2.talk_volume*5 + 46};
		audio_talk_open(ctrl);
	}
...
static void monitor_call_extern_func(unsigned long arg1, unsigned long arg2)
{
	...
	if (device == DEVICE_OUTDOOR_1)
	{
		...
		if (user_data_get()->other.model == AT_HOME_PATTERN)
		{
			...
			{
				printf("[%s:%d]====>> wxj debug, ringback:%d way:%d\n", __func__, __LINE__, user_data_get()->audio.ringback, monitor_enter_way_get());
...
static void monitor_call_inside_action(MONITOR_CH ch, unsigned long arg2)
{
	...
	int ring_time;
	ring_time = ch == MON_CH_DOOR_1 ? ring_attr.door1.ring_time : ring_attr.door2.ring_time;
...
static void monitor_call_inside_func(unsigned long arg1, unsigned long arg2)
{
	...
	if (/* (get_outdoor_talk_state(MON_CH_DOOR_1) || get_outdoor_talk_state(MON_CH_DOOR_2) )&& */ monitor_enter_way_get() == MONITOR_ENTER_CALL)
	{
	...
	if (local_ch != call_ch)
	{
		...
		audio_talk_ctrl ctrl = {{device}, (OPERATION_OPTION(AUDIO_SEND_EN) | OPERATION_OPTION(AUDIO_RECEIVE_EN)), AI_AO_C, true, false, call_ch == MON_CH_DOOR_1 ? user_data_get()->door1.talk_volume * 5 + 46 : user_data_get()->door2.talk_volume * 5 + 46};
		audio_talk_open(ctrl);
