...
typedef struct 
{
    ...
    UINT_T video_width;             ///<video width
    UINT_T video_height;            ///<video height
}RING_BUFFER_NODE_T;
 ...
 * @param[in]   video_height
 * @return error code
 * - OPRT_OK      success
 * - Others       failed
*/
OPERATE_RET tuya_ipc_ring_buffer_append_data_with_resolution(RING_BUFFER_USER_HANDLE_T handle, UCHAR_T *data_addr, UINT_T data_size, MEDIA_FRAME_TYPE_E dataType,
                                                                           UINT64_T pts, UINT_T video_width, UINT_T video_height);
 ...
 * @param[in]   pts          time stamp in us
 * @param[in]   timestamp    time stamp in ms
 * @param[in]   video_width  if the data is video frame
 * @param[in]   video_height if the data is video frame
 * @return error code
 * - OPRT_OK      success
 * - Others       failed
 */ 
OPERATE_RET tuya_ipc_ring_buffer_append_data_with_timestamp_and_resolution(RING_BUFFER_USER_HANDLE_T handle, UCHAR_T *data_addr, UINT_T data_size, MEDIA_FRAME_TYPE_E dataType,
                                                                                   UINT64_T pts, UINT64_T timestamp, UINT_T video_width, UINT_T video_height);
