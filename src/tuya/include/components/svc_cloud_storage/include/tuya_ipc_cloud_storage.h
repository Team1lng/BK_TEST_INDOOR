...
#define CLOUD_STORAGE_SYNC                  0
#define CLOUD_STORAGE_ASYNC                 1
#define CACHE_DIR_LEN                       64
...
typedef INT_T(*CLOUD_STORAGE_GET_OTHER_CLIENT_USER_NUM_CB)(VOID);

typedef struct {
    INT_T enable;
    INT_T pic_event_enable;                 // 事件图片续传使能
    CHAR_T offline_path[CACHE_DIR_LEN];
    UINT_T max_record_hours;
    CLOUD_STORAGE_GET_OTHER_CLIENT_USER_NUM_CB get_other_client_num_cb;

    INT_T data_file_need_backup;            // TRUE: 使能文件备份(会加重IO占用)
    UINT_T max_record_num;                  // 离线记录最大条数(default:4000)
    INT_T enable_intelli_resume;            // TRUE:开启根据上传网络状况智能调控续传
    UINT_T max_time_for_upload_to_cloud;    // 单条数据上传至云端的耗时阈值(default:10s，超过表示网络差，依赖 enable_intelli_resume)
    INT_T upload_control_enable;            // 续传上传控制, TRUE:预览时停止上传/FALSE:正常上传
} CLOUD_STORAGE_BREAKPOINT_T;

typedef struct 
{
    INT_T read_buf_len; // 上传离线云存储媒体文件的读buf长度限制(at least:64*1024)
    CHAR_T cloud_media_file_cache_dir[CACHE_DIR_LEN];
    INT_T upload_control_enable;        // 上传控制, TRUE:预览时停止上传/FALSE:正常上传
    CLOUD_STORAGE_BREAKPOINT_T breakpoint_cfg;
    BOOL_T allow_memory_caching;//允许在内存中额外缓存一个云存储录像片，用于设备在线录制时减少sd卡读写次数
}CLOUD_STORAGE_OFFLINE_RESUME_CONFIG;
...
* - Others  init failed
*/
OPERATE_RET tuya_ipc_cloud_storage_init();
...
OPERATE_RET tuya_ipc_cloud_storage_disable_async_mode(BOOL_T disable);

OPERATE_RET tuya_ipc_cloud_storage_init_with_offline_resume_config(IN CLOUD_STORAGE_OFFLINE_RESUME_CONFIG *upload_config);

BOOL_T tuya_ipc_cloud_storage_get_status_with_async(VOID);

OPERATE_RET tuya_ipc_cloud_set_send_sleep_time(UINT_T sleep_time_ms);
