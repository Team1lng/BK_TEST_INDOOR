...
typedef struct
{
    ...
    BOOL_T lan_only_mode;             /** work in lan only mode */
    UINT_T recv_buffer_size; /*recv app data size. if recv_buffer_size = 0,default = 16*1024*/
    BOOL_T limit_by_remote;  /*recv p2p limit num cmd from cloud*/
}MEDIA_STREAM_VAR_T;
...
/** @brief restrict stream module
 * @return error code
 * - OPRT_OK success
 * - Others  fail
 */
OPERATE_RET tuya_ipc_media_service_restrict();
