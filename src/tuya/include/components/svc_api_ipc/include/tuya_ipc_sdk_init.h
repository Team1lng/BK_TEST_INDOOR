...
typedef INT_T (*TUYA_SAVE_DB_KEY_CB)(BYTE_T *encrypt_key, UINT_T encrypt_key_len);
...
typedef struct {
    ...
    BOOL_T db_key_enable; ///<If set to TRUE, the database encrypt key will use the user-defined db_key
    BYTE_T db_key[IPC_DB_KEY_LEN + 1]; ///< database encrypt key
    TUYA_SAVE_DB_KEY_CB save_db_key_cb; ///< user saves db key. used to maintain compatibility with versions where users cannot set db key
