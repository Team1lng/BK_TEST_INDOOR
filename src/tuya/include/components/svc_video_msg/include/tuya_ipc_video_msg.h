...
/**
 * @brief push a alarm with video
 * 
 * @param[in] pic_data: pic data
 * @param[in] pic_len: pic len
 * @param[in] extra_data: extra data append to upload info, json format: "aaa":3. NULL if not need
 * @param[in] name:notification name, please refer to NOTIFICATION_NAME_E
 * 
 * @return OPRT_OK on success. Others on error, please refer to tuya_error_code.h 
 */
OPERATE_RET tuya_ipc_trigger_alarm_with_video(IN CHAR_T *pic_data, IN INT_T pic_len, IN CHAR_T *extra_data, IN NOTIFICATION_NAME_E name);
