...
STATIC VOID handle_DP_SD_STORAGE_ONLY_GET(IN TY_OBJ_DP_S *p_obj_dp)
{
    ...
    tuya_ipc_sd_get_capacity_by_mount_path(NULL, &total, &used, &empty);
...
STATIC VOID handle_DP_BLUB_SWITCH(IN TY_OBJ_DP_S *p_obj_dp)
{
    ...
    tuya_monitor_light_event(blub_on_off);
}
int tuya_dp_138_response_light_switch(bool state)
{
    ...
    BOOL_T param = state;
    respone_dp_bool(TUYA_DP_BLUB_SWITCH, param);
    return 0;
}
#endif
...
#ifdef TUYA_DP_REBOOT_SYSTEM
STATIC VOID handle_DP_REBOOT_SYSTEM(IN TY_OBJ_DP_S *p_obj_dp)
{
...
STATIC VOID handle_DP_WORK_MODE(IN TY_OBJ_DP_S *p_obj_dp)
{
    if ((p_obj_dp == NULL) || (p_obj_dp->type != PROP_ENUM))
    {
        PR_ERR("Error!! type invalid %d \r\n", p_obj_dp->type);
    ...
    tuya_work_mode_switch_event(p_obj_dp->value.dp_enum);
}
int tuya_dp_189_response_work_mode(unsigned int mode)
{
    ...
    if (mode >= (sizeof(param) / sizeof(param[0])))
    {
        return -1;
    }
    tuya_ipc_dp_report(NULL, TUYA_DP_WORK_MODE, PROP_ENUM, (void *)param[mode], 1);
    return 0;
}
#endif
...
STATIC VOID handle_DP_SWITCH_CHANNEL(IN TY_OBJ_DP_S *p_obj_dp)
{
    ...
    {
        db_log_error("Channel %d invalid \n", channel);
        return;
    }
    extern bool tuya_monitor_swap_event(int ch);
    tuya_monitor_swap_event(channel);
}
...
int tuya_channel_report(int curr_ch, tuya_ch_info_t *info, int total)
{
    ...
    // db_log_debug("%s\n", dp_str);
    respone_dp_str(TUYA_DP_SWITCH_CHANNEL, dp_str);
    return 0;
}
#endif
...
int tuya_lock_support_report(void)
{
    ...
    dp_str[strlen(dp_str) - 1] = '\0';
    respone_dp_str(TUYA_DP_LOCK_SUPPORT, dp_str);
	return 0;
}
...
int tuya_dp_235_response_device_active(void)
{
    ...
    BOOL_T param = false;
    respone_dp_bool(TUYA_DP_DEVICE_ACTIVE, param);
    return 0;
}
#endif

#ifdef TUYA_DP_DEVICE_SCREENSHOT
STATIC VOID handle_DP_DEVICE_SCREENSHOT(IN TY_OBJ_DP_S *p_obj_dp)
{
...
int tuya_screenshot_report(bool state)
{
    ...
    respone_dp_bool(TUYA_DP_DEVICE_SCREENSHOT, state);
    return 0;
}
#endif
...
#ifdef TUYA_DP_REBOOT_SYSTEM
        {TUYA_DP_REBOOT_SYSTEM, handle_DP_REBOOT_SYSTEM},
#endif

#ifdef TUYA_DP_WORK_MODE
        {TUYA_DP_WORK_MODE, handle_DP_WORK_MODE},
#endif
...
#ifdef TUYA_DP_INDOOR_LOCK
        {TUYA_DP_INDOOR_LOCK, handle_DP_INDOOR_LOCK},
#endif

#ifdef TUYA_DP_DEVICE_SCREENSHOT
        {TUYA_DP_DEVICE_SCREENSHOT, handle_DP_DEVICE_SCREENSHOT},
#endif
